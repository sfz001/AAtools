...
4. 失败时启用视频模式：`handleTranscribeVideo` **优先用 sub2api #3**（用户约定的 Gemini 中转槽位）走 `{baseUrl}/v1beta/models/{model}:streamGenerateContent`，model 取 `sub2api3Model`（必须是 `gemini-*`）否则 fallback 到 `gemini-2.5-flash`；sub2api #3 缺 key 或 baseUrl 时回落到原生 Gemini provider 走 `gemini-flash-lite-latest`；两条都没配则报错
