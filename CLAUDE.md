...
- **模型验证**: `sanitizeModel(provider, model)` 检查模型前缀（`claude-`/`gpt-`/`gemini-`），不匹配则静默回退到默认模型；claude 额外用 `RETIRED_CLAUDE` 正则拦截已退役的 Claude 2.x / 3.x / instant 系列（存量配置命中时清空回退默认模型，避免 404）；minimax / sub2api / sub2api2 / sub2api3 跳过严格校验（sub2api 系列同时支持 `claude-*` / `gemini-*` / `gpt-*`）
